...
def aruco_callback(msg):
    ...
    if current_aruco_id != previous_aruco_id and current_aruco_id in [15, 16]:
        if i == True:
            time.sleep(3)
        calculate_move_left()
    elif current_aruco_id != previous_aruco_id and current_aruco_id == 17:
        stop_robot()
...
def calculate_move_left():
    ...
    if initial_orientation is not None:
        ...
        # This segment is just to make-up for our absent <Orienation> aspect, once we do it we won't be needing this
        if (0 <= current_position[0] <= 45): initial_yaw = 0
        elif (45 < current_position[0] <= 90): initial_yaw = 90
        elif (90 < current_position[0] <= 180): initial_yaw = 180
        elif (-90 <= current_position[0] < 180): initial_yaw = -90
        elif ( 0 <= current_position[0] < -90): initial_yaw = 0
        ...
        rospy.loginfo(f"Current bot position: x={current_position[0]:.2f}, y={current_position[1]:.2f}, yaw={initial_yaw:.2f}")
